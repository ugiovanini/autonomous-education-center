...
import { disciplines, sampleCourses, disciplineCourses, disciplineCourseGroups } from "@/data/courses";
...
const Disciplines = () => {
  ...
  if (slug) {
    ...
    const courseGroups = disciplineCourseGroups[slug];
              ...
              <section>
                {courseGroups.map((g) => (
                  <div key={g.group} className="mb-10">
                    <h2 className="font-heading font-semibold text-xl text-foreground mb-4">{g.group} ({g.courses.length})</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {g.courses.map((name, i) => (
